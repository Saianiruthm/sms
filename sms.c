...
void generate_student_id(char student_id[]) {
...
"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
 ...
 srand(time(NULL));
 // Generate the random student ID
 for (int i = 0; i < id_length; i++) {
 student_id[i] = characters[rand() % (sizeof(characters) - 1)];
 }
 student_id[id_length] = '\0';
}
FILE *file,*f1;
...
void remove_student()
{
 ...
 printf("\n\t\tEnter Student name that to be delete:");
 scanf("%s",id);
 int check=0;
 rewind(file); //move file position indicator to the beginning
 while((fread(&std,sizeof(std),1,file))==1)
 {
 if(strcmp(std.name,id)==0)
 {
 check=1;
 }
 else
...
void add_new_student()
{
 ...
 printf("\n\t\t\t\tEnter Last name:");
 scanf("%s",std.last_name);
 generate_student_id(std.student_id);
 printf("\n\t\t\t\tIf Female write F or If Male write M:");
 scanf("%s",std.gender);
 getchar();
 printf("\n\t\t\t\tEnter Father name:");
 ...
 printf("\n\t\t\t\tEnter Date of Birth:");
 scanf("%s",std.date_of_birth);
 printf("The Generated Student ID is %s.", std.student_id);
 fwrite(&std,sizeof(std),1,file);
...
void display_student()
{
 ...
 while(c!=NULL)
 {
 printf("\t\t\t\t\t%s %s\t\t%s\n",c->name,c->last_name,c->student_id);
 c=c->next;
 }
...
//printf("**************************************************************************
**********************************************\n\n");
 //printf("\t\t\t\t\tGovernment College Of Technology\n\n");
